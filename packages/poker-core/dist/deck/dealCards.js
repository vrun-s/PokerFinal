...
export function dealCards(deck, count) {
    if (count < 0) {
        throw new PokerError("INVALID_DEAL_COUNT", "Cannot deal a negative number of cards.");
    }
    if (deck.length === 0 && count > 0) {
        throw new PokerError("EMPTY_DECK", "Cannot deal cards from an empty deck.");
    }
    if (count > deck.length) {
        throw new PokerError("INSUFFICIENT_CARDS", `Cannot deal ${count} cards. Only ${deck.length} cards remaining in the deck.`);
    }
