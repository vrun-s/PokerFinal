...
export function rankValue(rank) {
    ...
    if (val === undefined) {
        throw new PokerError("INVALID_ARGUMENT", `Invalid card rank: ${rank}`);
    }
    return val;
}
/**
...
export function serializeCard(card) {
    ...
    if (!suitChar) {
        throw new PokerError("INVALID_ARGUMENT", `Invalid suit when serializing card: ${card.suit}`);
    }
...
export function parseCard(str) {
    if (typeof str !== "string" || str.length !== 2) {
        throw new PokerError("INVALID_CARD_SERIALIZATION", `Invalid card serialization string (must be 2 characters): "${str}"`);
    }
    const rankStr = str[0];
    const suitChar = str[1];
    if (!rankStr || !suitChar) {
        throw new PokerError("INVALID_CARD_SERIALIZATION", `Failed to extract rank and suit from string: "${str}"`);
    }
    const rank = RANKS.find((r) => r === rankStr);
    const suit = CHAR_TO_SUIT[suitChar];
    if (!rank || !suit) {
        throw new PokerError("INVALID_CARD_SERIALIZATION", `Invalid rank "${rankStr}" or suit "${suitChar}" in string: "${str}"`);
    }
    return { rank, suit };
}
/**
