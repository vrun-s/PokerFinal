...
export function combinations(arr, k) {
    if (arr.length === 0) {
        throw new PokerError("INVALID_COMBINATION_PARAMS", "Cannot generate combinations from an empty array.");
    }
    if (k <= 0) {
        throw new PokerError("INVALID_COMBINATION_PARAMS", `Combination size k must be positive. Received k = ${k}.`);
    }
    if (k > arr.length) {
        throw new PokerError("INVALID_COMBINATION_PARAMS", `Combination size k (${k}) cannot exceed array length (${arr.length}).`);
    }
    const results = [];
