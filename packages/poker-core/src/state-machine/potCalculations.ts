...
import { PayoutResult, Payout } from "../types/PayoutResult.js";
import { bestHand } from "../evaluation/bestHand.js";
import { compareMany, PlayerHand } from "../evaluation/compareMany.js";

/**
 * Checks if two player ID lists contain the exact same elements (order-independent).
 */
function haveSameElements(arr1: readonly string[], arr2: readonly string[]): boolean {
 ...
 */
export function calculatePots(players: readonly PlayerState[]): readonly Pot[] {
...
export function distributePayouts(
  ...
  const bestHandsMap: Record<string, any> = {};
  const getBestHand = (id: string) => {
    if (!bestHandsMap[id]) {
  ...
  for (const pot of pots) {
    ...
    const potHands: PlayerHand[] = eligibleActiveIds.map(id => ({
      playerId: id,
  ...
  const payouts: Payout[] = Object.entries(payoutsMap).map(([playerId, amount]) => ({
    playerId,
    amount,
  }));
